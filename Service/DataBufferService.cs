	...
	public class DataBufferService
	{
		...
		private async Task ToPLCClasses(IEnumerable<Plcconfig> plcconfigs)
		{
			do
			{
				...
				//});
			} while (plcClasses.Count != plcconfigs.Count());
			OnPLCClassesChange();
		}
			...
			await Task.Run(async () =>
			{
				using (var scope = scopeFactory.CreateScope())
				{
					...
					if (taskExist)
					{
						...
						target.LastLog = mesTask.LastLog;
						target.CustomInfo = mesTask.CustomInfo;
		...
		public async Task SwarmCoreUpdateStatusFailPauseWithTimeLog(MesTaskDetail mesTask, int status, string log)
		{
		...
		public async Task<(bool, string)> CancelWIPTask(string tasknoFromMes, string custonField)
		{
		...
		public async Task<(bool, string)> ForceCancelWIPTask(string tasknoFromMes, string customField)
		{
			//await UpdateSwarmCoreTaskStatus();
			if (!MesTasks_WIP.Exists(x => x.TaskNoFromMes == tasknoFromMes))
			{
			...
			else
			{
				...
				else
				{
					...
					await RemoveFromWIP(target, reason, target.LastLog, customField);
					MesTasks_WIP.Remove(target);
					OnMesTaskCancel(target);
					return (true, reason);
				}
				...
				else
				{
					if (swarmCoreTaskStatus.Exists(x => x.flowid == tmp.TaskNoFromSwarmCore))
					{
						...
						switch (step)
						{
							...
							case "handshake1":
								return "A003";
							case "handshake1_Astart":
								return "A010";
							case "handshake1_Lstart":
								return "A011";
							case "handshake1_Afinish":
								return "A012";
							case "handshake1_Lfinish":
								return "A013";
							case "handshake1_Areset":
								return "A014";
							case "handshake1_Lreset":
								return "A015";
							case "barcodecheck1":
							...
							case "handshake2":
								return "A008";
							case "handshake2_Astart":
								return "A020";
							case "handshake2_Lstart":
								return "A021";
							case "handshake2_Afinish":
								return "A022";
							case "handshake2_Lfinish":
								return "A023";
							case "handshake2_Areset":
								return "A024";
							case "handshake2_Lreset":
								return "A025";
							case "moving3":
								return "A009";
							default:
		...
		public async Task<(bool, string)> GetReviseTask(ReviseTask reviseTask)
		{
			...
			try
			{
				...
				if ((statusCode == 5550000 || statusCode == 200))
				{
					...
					Log.Information("mesTask " + mesTask.TaskNoFromMes + " is initilized and upsert to WIP");
					OnSingleMesTaskChange(mesTask);
					await CancelWIPTask(originalTask.TaskNoFromMes, "from reverse");
					...
					if (swarmCoreTaskStatus.Any(x => x.flowid == mesTask_WIP.TaskNoFromSwarmCore))
					{
						if (swarmCoreTaskStatus.Any(x => x.flowid == mesTask_WIP.TaskNoFromSwarmCore && x.state != mesTask_WIP.Status))
						{
							...
							switch (mesTaskState.state)
							{
								...
								case 3://fail
								case 4://pause
									await SwarmCoreUpdateStatusFailPauseWithTimeLog(mesTask_WIP, mesTaskState.state, mesTaskState.status_msg);
									break;
		...
		public async Task RemoveFromWIP(MesTaskDetail mesTask_WIP, string reason, string lastLog, string customInfo)
		{
			...
			mesTask_WIP.LastLog = lastLog;
			mesTask_WIP.CustomInfo = customInfo;
